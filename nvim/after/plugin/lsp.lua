if not vim.g.vscode then
    ...
    lsp.on_attach(function(client, bufnr)
        ...
        vim.keymap.set("n", "<leader>vS",
            function() require('telescope.builtin').lsp_dynamic_workspace_symbols({ symbol_width = 60,
                    symbol_type_width = 30, fname_width = 50 }) end,
            { buffer = bufnr, remap = false, desc = "Show current Workspace Symbols" })
        vim.keymap.set("n", "<leader>vs",
            function() require('telescope.builtin').lsp_document_symbols({ symbol_width = 60, symbol_type_width = 30,
                    fname_width = 80 }) end, { desc = "Show symbols in document", buffer = bufnr, remap = false })
        vim.keymap.set("n", "<leader>vr", function() require('telescope.builtin').lsp_references({ fname_width = 80 }) end,
        ...
        vim.keymap.set("n", "<leader>vrn", ":lua vim.lsp.buf.rename()<CR>", { desc="rename symbol", buffer = bufnr, remap = false })
        ...
        vim.keymap.set('n', '<leader>dd',
            function() if vim.diagnostic.is_disabled() then vim.diagnostic.enable() else vim.diagnostic.disable() end end, {desc="Toggle diagnostics"})
        vim.keymap.set("n", "<leader>vD", function() vim.diagnostic.open_float() end, { buffer = bufnr, remap = false, desc="Show diagnostics window" })
